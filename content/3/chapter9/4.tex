...
运行Memcheck的开销很大。手册(参见扩展阅读中的链接)里说，它可使程序慢10-15倍。为了避免每次运行测试时都等待Valgrind，将创建一个单独的目标，在需要测试代码时从命令行调用该目标。理想情况下，开发人员将在将更改合并到存储库的默认分支之前运行它。这可以通过Git钩子完成，也可以作为CI流水中的一个步骤。要构建一个自定义目标，需要在生成阶段完成后使用以下命令:
