...
\subsubsubsection{12.4.2\hspace{0.2cm}构建Calc Console可执行文件}
...
你可能还记得在第5章中，可以使用CMake来捕获一些构建时的值，并通过模板文件将其提供给C++代码——例如，使用一个C++结构体:
...
包含该模块将设置包含所需要的信息的变量，然后使用configure\_file()来生成buildinfo.h。剩下的就是调用BuildInfo函数，并添加生成文件的目录，以包含所需目标的目录。然后，可以与多个不同的使用者共享该文件。这种情况下，可能需要在列表文件的顶部添加include\_guard(GLOBAL)，以避免对每个目标都运行git命令。

深入研究控制台计算器的实现之前，不应该过分担心tui.cpp文件的复杂性。要完全理解它，只需要对FXTUI库有一定的了解——这里我们不想太深入。相反，让我们关注突出显示的行:
